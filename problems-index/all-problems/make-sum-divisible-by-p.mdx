...
Now, for the case where M(j) == target_mod, which would mean removing [0,j],  length is j+1, so initialize the hash with &lbrace;0=&gt;-1&rbrace;. And also this case cant happen at j==n-1, which would mean we are removing the complete array.
</Card>
